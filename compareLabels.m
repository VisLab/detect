function [results errorInfo timeInfo] = compareLabels(inputData, labeledSet1, labeledSet2, timingError, srate)
...
elseif ndims(inputData) == 2 && length(inputData) ~= 1 %#ok<ISMAT>
    ...
    error('compareLabels:DataNot2D','Data must be 2-D')
end
...
for i = 1 : size(labeledSet1, 1)
    ...
    indices_1 = SplitVec(region_1, 'equal', 'firstval');
    
    % grab the location and type of events found in the region
    loc_2 = SplitVec(region_2, 'equal', 'loc');
    indices_2 = SplitVec(region_2, 'equal', 'firstval');
    ...
    disagreements_regions = SplitVec(disagreements, 'equal', 'loc');
    disagreements_type = SplitVec(disagreements, 'equal', 'firstVal');
...
unique_regions = SplitVec(agreement_vector, 'equal', 'loc');
unique_regions_type = SplitVec(agreement_vector, 'equal', 'firstval');

% the confusion matrix is size (nLabels) + 1 (for the null event).
% use the confusion matrix to calculate errorInfo.
MatrixLabels = {'Null' labels{:}}';
